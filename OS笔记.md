...
CPU首先会自动从软盘的头部开始读入521字节的MBR，MBR的要求是最后两个字节必须是`0xaa55`。

boot代码就是一个MBR。boot代码的主要功能只有一个，就是把loader加载进入内存。系统刚启动处在实模式，可以调用BIOS功能，包括输出。
...
![1565002604851](images/1565002604851.png)
...
因为要把boot作为FAT12文件系统的MBR，所以它必须满足FAT12文件系统的一些格式要求(虽然其实这些工具都是我们自己写的，完全可以不遵循)

这一部分确实有点难，对于我这样的新手来说，但是我已经写完了，就不再赘述怎么写了。

#### Loader

跳转进入loader之后，除了给出一些提示信息之外，接下来要做的就是把内核加载进入内存，因为内核一般比较大，所以实模式下的1M空间肯定不够，所以我们需要保护模式来访问4GB内存空间，但是同时又想使用BIOS的磁盘驱动器来读软盘，所以此时一个不错的选择是使用Big real mode，这个模式介于实模式和保护模式之间，关于开启方式和介绍，可以看看[这一篇博客](https://www.cnblogs.com/sinbad-li/p/5019351.html)

开启big real mode模式成功了之后，继续通过FAT12加载工具将内核加载进入内存。
...
现在我准备重新设计一下bootloader，主要的改进在于稍微规范一下汇编代码设计，采用函数式的设计，同时通过堆栈的使用，统一传参方式，不再需要约定任何寄存器。
...
虽然有64位，但是intel规定只有其中48位用来寻址，高16位属于符号扩展位，并且高16位的值和第47位保持一致(从0位开始)，所以，有效地址就变成了这样的：
...
现在定义的启动用的磁盘是软盘，至于为什么用软盘，可能只是因为Bochs提供了软盘驱动器？也许吧。
...
BIOS读取磁盘的时候采用CHS格式的编号，也就是说我们需要把扇区编号转换为：柱面，磁头，扇区
...
16位模式又称为实模式，我的理解是，他表示的逻辑地址直接就是物理地址，所以称之实模式。32为模式常称为保护模式，接下来是64位模式。大概粗略的分为这三种情况，接下来说一下这三种模式下，是怎么表示内存地址的。
...
这种模式下，地址位宽64位，但是并不是全用来表示地址。其寻址能力只有低48bit，高16位是符号扩展，要么全0，要么全1，所以明显，可以表示地址的只有整个64位地址空间的首尾两小段。称之Canonical地址。中间的无效地址称为Non-canonical地址。
...
在页模式下，64bit线性地址依旧只有低48位有作用，其中39到47这9位负责对于四级页表PML4(page map level 4)的索引，30到38这9位负责PDPT(page directory pointer table)，下九位负责PDT，再九位是PT，最后12位是页内偏移。

CR3负责PML4的基址，PLM4里面的项是PML4E(E代表Entry)，PDPT的项是PDPTE，PDT里面是PDE，PT里面是PTE
...
![1566451862692](images/OS笔记/1566451862692.png)
...
![1566453161618](images/1566453161618.png)
