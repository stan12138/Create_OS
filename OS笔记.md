...
首先，ELF文件前面有魔数，据观察PE文件也有很短的一个相同的头部，但是是什么意思，一直不太清楚，今天认真看了一下[微软官网的解析](https://docs.microsoft.com/en-us/windows/win32/debug/pe-format#machine-types)(Ps.这里是微软定义的PE格式的详细结构)，竟然发现对上了.....
...
主要参考资料：主要是`professional assembly language`即汇编语言程序设计这本书，也可以看看[这个网页](https://docs.huihoo.com/gnu_linux/own_os/preparing-asm_4.htm)内容基本一致，但是至少比较清晰。
...
前面按道理来说，我认为已经把内核头的知识写的差不多了，并且现在的内核头依旧还是直接使用书里面作者的代码，这里就大概描述一下一个内核头应该配备什么，为接下来自己写内核头做个铺垫。

#### 基础版内核头

首先需要设置一下GDT表，按照书里面的做法，会设置两个64位代码段，一个内核的一个用户的，两个64位数据段，也是这样分配。还有两个内核的32位段。

然后要设置页表，当然这里暂时只需要设置少数几个页表就行。
...
载入GDTR和IDTR
重设段寄存器和栈
接下来设置CR3寄存器，并通过一次模拟返回跳转到内核段线性地址
然后再次重设段寄存器和栈
然后再通过模拟返回跳转到内核代码
~~~
...
在上述基础内核头的基础上，还需要在跳转进入内核之前设置IDT和TSS

~~~
构造IDT，然后循环设置全部256个IDT表项
构造TSS描述符结构，然后在GDT里面设置TSS描述符
最后构建一个简单的通用中断处理函数，暂时这个函数只需要输出一行信息就行。
~~~
